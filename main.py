...
def suggestions(complexity, issues):
    s = []
    if complexity > 10: s.append("Reduce function complexity.")
    if issues > 0: s.append("Fix detected code style issues.")
    if not s: s.append("Code looks good.")
    return s
...
async def node_suggest(state):
    ...
    # derive quality_score (same as your logic)
...
GRAPHS = {}  #used for storing graphs
RUNS = {}  #used for storing runs 
...
@app.post("/graph/create")    #create a new graph endppoint
